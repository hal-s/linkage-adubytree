...
$.fn.AdubyTree = function(config ) { 
	...
	var adubyTree ={
		...
		parseNode : function (node) {
			...
			if(node.haschild && node.isfolder == 1){
			...
			if(node.selected){
				str	+= 	"<a id='"+this.elemId+"-"+node.id+"' name='"+node.id+"' href='"+ node.jsaction +"'  data='"+this.object2JsonString(node)+"'   pid='"+node.pid+"'   class='node selected " + typeClass + "' nowrap>"+node.data + "</a>";
			}else{
				str	+= 	"<a id='"+this.elemId+"-"+node.id+"' name='"+node.id+"' href='"+ node.jsaction +"'  data='"+this.object2JsonString(node)+"'   pid='"+node.pid+"'   class='node " + typeClass + "' nowrap>"+node.data + "</a>";
			}
			if(node.open){
		...
		parseJSON : function (node_id,data,indent,pNode) {
			...
			// 	debugger;
			// }
			if(data.children && data.children.length == 0 && data.isfolder == 1) { //空文件夹
				return str;
			}else if((data.children && data.children.length ||data.haschild=="true"||data.haschild)&&node_id){//not root
				if(data.children && data.children.length){
					for(var i = 0; i < data.children.length; i++) {
						(i == data.children.length -1) ? indent.push(1) : indent.push(0);
						data.children[i].order=	i+1;
						data.children[i].pid=data.id;
						data.children[i].pNode=data;
						if(node_id == data.id){
		...
		//自添加函数
		//根据路径找节点
		findNodeByPath : function(wjName){
			data = data || this.config.data;
		},
		//自添加函数
		...
		object2JsonString: function(obj){
			var json	=	"{";
			for (variable in obj){
				if(obj[variable]!=null&&obj[variable].constructor!=Array&&Object.prototype.toString.call(obj[variable]) !== '[object Object]'){
		...
		loadTree	: function(data){
			// $(this.elem).html(this.parseJSON(data));
			$(this.elem).html(this.parseJSON());//更改代码
			this.open();
			//this.applyCookie();
		}
	};
	...
	$.fn.extend({
		...
		openAll : function (data) {
			...
			return this.each( function() {
				if(this.adubytree) return;
				this.adubyTree.openAll(data);
			});
		},
		openSelf : function (data) {
			return this.each( function() {
				if(this.adubytree) return;
				this.adubyTree.openSelf(data);
			});
		},
		closeAll : function () {
			return this.each( function() {
				if(this.adubytree) return;
				this.adubyTree.closeAll(data);
			});
		},
		closeSelf : function (data) {
			return this.each( function() {
				if(this.adubytree) return;
				this.adubyTree.closeSelf(data);
			});
		},
			...
			if(_this.adubyTree.config.url){//data from remote url
				$.ajax({url: _this.adubyTree.config.url,type: _this.adubyTree.config.type,dataType: _this.adubyTree.config.dataType,data:{ id : _this.adubyTree.config.id},timeout: 15000,async: false,
					...
					success: function(data, textStatus){
						...
						_this.adubyTree.config.data = data
